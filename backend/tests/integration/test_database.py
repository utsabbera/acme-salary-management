...
async def test_scd_type2_resolution(db_session: AsyncSession) -> None:
    employee = Employee(
        first_name="John",
        last_name="Doe",
...
async def test_soft_deletes(db_session: AsyncSession) -> None:
    employee = Employee(
        first_name="Jane",
        last_name="Doe",
...
async def test_cascade_deletes(db_session: AsyncSession) -> None:
    employee = Employee(
        first_name="Bob", last_name="Smith", email="bob@example.com", department="HR", country="USD"
    )
    db_session.add(employee)
    await db_session.flush()
